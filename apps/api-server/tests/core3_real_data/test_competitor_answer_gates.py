...
def test_price_nearest_candidate_with_insufficient_anchor_cannot_rank_first() -> None:
    ...
    assert "anchor_substitutability_below_primary_threshold" in nearest["ranking_gate_reasons"]


def test_weak_market_validation_with_deviated_purchase_pool_is_excluded_from_top3() -> None:
    answer = build_competitor_answer(
        target=_target(),
        target_fact_brief=_target_fact_brief(),
        competitors=[
...
def test_low_replacement_pressure_blocks_direct_role_upgrade() -> None:
    ...
    assert "replacement_pressure_below_strong_threshold" in low_pressure["ranking_gate_reasons"]
    ...
    *,
    ...
    return {
        ...
        "value_anchor": {
            ...
            "candidate_stronger_anchors": ["画质配置解释加价"] if anchor_score >= 12 else [],
            "anchor_substitutability_score": anchor_score,
            "anchor_substitutability_level": "strong" if anchor_score >= 13 else "partial",
        ...
        "replacement_pressure": {
            ...
            "replacement_pressure_level": "high" if replacement_score >= 8 else "medium" if replacement_score >= 5 else "low",
