...
from app.services.core3_real_data.task_seed_loader import M09TaskSeedError
from app.services.core3_real_data.user_task_repositories import M09InputBlockedError, M09UserTaskRepository
from app.services.core3_real_data.user_task_schemas import M09ServiceResult, M09SkuTaskReviewIssueRecord
from app.services.core3_real_data.user_task_service import UserTaskService


class UserTaskRunner:
    module_code = Core3ModuleCode.M09
    ...
    def run(self, context: Core3RunContext, target: Core3ModuleTarget) -> Core3ModuleRunResultSchema:
        ...
        if not batch_id:
            return _blocked_result(
                ...
                message_cn="M09 缺少 M00 batch_id，无法生成用户任务。",
        ...
        return self.run_batch(
            ...
            rule_version=str(target.metadata.get("rule_version") or CORE3_M09_RULE_VERSION),
        )
        ...
        *,
        ...
        try:
            with self.db.begin_nested():
                service_result = UserTaskService(M09UserTaskRepository(context)).run_batch(
                    batch_id=batch_id,
                    run_id=run_id,
        ...
        except (M09InputBlockedError, M09TaskSeedError, ValueError) as exc:
            return _blocked_result(
                project_id=project_id,
                category_code=category_code,
                batch_id=batch_id,
        ...
        summary_json = {
            ...
            "category_code": category_code,
            "run_id": run_id,
            "module_version": CORE3_M09_MODULE_VERSION,
            "target_sku_codes": list(sku_scope),
            **service_result.summary,
        }
        output_hash = stable_hash(summary_json, version="m09_user_task_summary_v1")
...
def _review_issues(result: M09ServiceResult) -> list[Core3ReviewIssueSchema]:
    return [_review_issue_schema(issue) for issue in result.review_issues[:100]]


def _review_issue_schema(issue: M09SkuTaskReviewIssueRecord) -> Core3ReviewIssueSchema:
    return Core3ReviewIssueSchema(
        issue_code=issue.issue_type,
        issue_type="m09_user_task_review",
        severity=_review_severity(issue.issue_severity),
        source_module=Core3ModuleCode.M09,
        object_type="sku_task_score",
        target_sku_code=issue.sku_code,
        evidence_refs=issue.evidence_refs_json,
...
def _downstream_impacts(result: M09ServiceResult) -> list[dict[str, object]]:
    return [
        {
            "sku_code": score.sku_code,
            "task_code": score.task_code,
            "module_codes": ["M10", "M11", "M12", "M13", "M14", "M15"],
            "data_domains": [Core3DataDomain.TASK.value],
            "reason_cn": "M09 用户任务关系、证据拆分或复核状态发生变化",
        }
