...
SERVICE_SUBDIMENSION_CODES = ("service_delivery_install", "service_fulfillment_excluded")
    ...
    return M10CTargetGroupTaxonomy(
        ...
        target_groups=(
            _tg(
                ...
                ("audience_child_family", "use_living_room_cinema", "picture_clarity_resolution", "audio_quality", "system_smooth_ads"),
                ("家庭", "客厅", "全家", "追剧", "日常看", "看电视"),
                ("medium_46_59", "large_60_69", "xlarge_70_85"),
                ("low", "mid_low", "mid", "mid_high"),
                ("tv_claim_theater_scene", "tv_claim_hdr_high_brightness", "tv_claim_speaker_sound", "tv_claim_eye_care_display", "tv_claim_voice_control"),
                ("screen_size_inch", "resolution_class", "hdr_support_flag", "memory_capacity_gb", "speaker_power_w"),
            ...
            _tg(
                ...
                ("replacement_source", "appearance_size_fit", "use_living_room_cinema", "value_price"),
            ...
            _tg(
                ...
                ("picture_clarity_resolution", "picture_brightness_hdr", "picture_color_accuracy", "picture_local_dimming_black", "audio_quality"),
                ("画质", "亮度", "控光", "色彩", "黑位", "miniled", "oled", "音效"),
                ("large_60_69", "xlarge_70_85"),
                ("mid_high", "high"),
                ("tv_claim_miniled_display", "tv_claim_qd_miniled_display", "tv_claim_rgb_miniled_display", "tv_claim_oled_self_lit", "tv_claim_hdr_high_brightness", "tv_claim_wide_color_accuracy", "tv_claim_local_dimming", "tv_claim_picture_engine_ai", "tv_claim_dolby_audio_video"),
                ("display_tech_class", "declared_brightness_nit_or_band", "local_dimming_zone_count", "color_gamut_percent", "picture_engine_chip", "speaker_power_w"),
            ...
            _tg(
                ...
                ("use_living_room_cinema", "appearance_size_fit", "appearance_slim_wall", "audio_quality"),
                ...
                ("screen_size_inch", "display_tech_class", "declared_brightness_nit_or_band", "local_dimming_zone_count", "speaker_power_w", "flush_wall_mount_flag"),
            ...
            _tg(
                ...
                CANONICAL_SIZE_TIERS,
                ("low", "mid_low", "mid"),
                ("tv_claim_value_price", "tv_claim_theater_scene", "tv_claim_high_refresh_rate", "tv_claim_hdr_high_brightness"),
                ("screen_size_inch", "price_per_inch", "resolution_class", "declared_brightness_nit_or_band", "declared_refresh_rate_hz"),
            ...
            _tg(
                ...
                ("游戏", "主机", "ps5", "xbox", "看球", "体育", "高刷", "低延迟", "流畅"),
                ...
                ("declared_refresh_rate_hz", "hdmi21_flag", "memc_motion_flag", "vrr_allm_flag", "memory_capacity_gb"),
            ...
            _tg(
                ...
                ("audience_child_family", "picture_eye_care_reflection", "use_living_room_cinema"),
                ("孩子", "儿童", "护眼", "不刺眼", "长时间", "眼睛", "舒服"),
                ("small_32_45", "medium_46_59", "large_60_69"),
                ("mid_low", "mid", "mid_high", "high"),
                ("tv_claim_eye_care_display", "tv_claim_hdr_high_brightness", "tv_claim_wide_color_accuracy"),
                ("eye_care_certification_flag", "low_blue_light_flag", "flicker_free_flag", "declared_brightness_nit_or_band", "declared_refresh_rate_hz", "hdr_support_flag"),
                adjacent_size_tiers=("xlarge_70_85",),
            ...
            _tg(
                ...
                ("audience_senior", "interaction_voice_casting", "system_smooth_ads", "use_living_room_cinema"),
            ...
            _tg(
                ...
                ("audience_rental_room", "use_bedroom", "value_price", "appearance_size_fit"),
                ("卧室", "租房", "宿舍", "第二台", "小房间", "副屏", "小尺寸"),
                ("small_32_45",),
                ("low", "mid_low"),
                ("tv_claim_value_price", "tv_claim_full_screen_design", "tv_claim_voice_control"),
                ("screen_size_inch", "resolution_class", "smart_tv_flag", "voice_recognition_flag", "price_per_inch"),
            ...
            _tg(
                ...
                ("wifi_builtin_flag", "network_tv_flag", "smart_tv_flag", "ai_large_model_flag", "voice_recognition_flag", "far_field_voice_flag", "iot_control_flag", "camera_builtin_flag", "memory_capacity_gb"),
                adjacent_size_tiers=("small_32_45",),
                adjacent_price_bands=("mid_low",),
            ),
        ),
...
class M10CTargetGroupTaxonomyLoader:
    def load(self, taxonomy_version: str, *, product_category: str) -> M10CTargetGroupTaxonomy:
        normalized_category = str(product_category or "").upper()
        if normalized_category == "TV" and taxonomy_version == CORE3_M10C_TV_TAXONOMY_VERSION:
            return tv_target_group_taxonomy_v0_1()
        raise ValueError(f"{normalized_category or product_category} 目标客群 taxonomy 未发布，不能生成 M10C 目标客群画像。")
...
class M10CTargetGroupRepository(ParamExtractionRepository):
    ...
    def save_scores(self, scores: Sequence[Any], *, replace_on_hash_conflict: bool = False) -> ParamRepositoryWriteResult:
        return self._save_many(
            ...
            scores,
            unique_fields=("batch_id", "taxonomy_version", "sku_code", "target_group_code", "rule_version", "is_current"),
    ...
    def save_coverages(self, coverages: Sequence[Any], *, replace_on_hash_conflict: bool = False) -> ParamRepositoryWriteResult:
        return self._save_many(
            ...
            coverages,
            unique_fields=("batch_id", "taxonomy_version", "target_group_code", "rule_version", "is_current"),
...
class M10CRunner:
    ...
    def run(self, context: Core3RunContext, target: Core3ModuleTarget) -> Core3ModuleRunResultSchema:
        ...
        return self.run_batch(
            ...
            taxonomy_version=str(target.metadata.get("taxonomy_version") or CORE3_M10C_TV_TAXONOMY_VERSION),
            rule_version=str(target.metadata.get("rule_version") or CORE3_M10C_TV_RULE_VERSION),
    ...
    def run_batch(
        ...
        *,
        ...
        return Core3ModuleRunResultSchema(
            ...
            output_count=service_result.profile_count + service_result.score_count + service_result.coverage_count,
            output_hash=stable_hash(summary_json, version="m10c_target_group_summary_v1"),
            warnings=service_result.warnings,
            review_issues=[],
            downstream_impacts=[
                {"module_code": "M11C", "reason": "目标客群变化会影响价值战场解释和后续竞品库。"},
                {"module_code": "M12", "reason": "目标客群变化会影响竞品召回和候选池解释。"},
            ],
...
class M10CService:
    ...
    def build_batch(
        ...
        *,
        ...
        taxonomy = M10CTargetGroupTaxonomyLoader().load(taxonomy_version, product_category=product_category)
        ...
        write_results = {
            "target_group_profiles": repository.save_profiles(profiles, replace_on_hash_conflict=force_rebuild),
            "target_group_scores": repository.save_scores(scores, replace_on_hash_conflict=force_rebuild),
            "target_group_coverages": repository.save_coverages(coverages, replace_on_hash_conflict=force_rebuild),
        }
        warnings: list[str] = []
        if not sku_inputs:
            warnings.append("M10C 没有读取到 M03B 参数画像，无法生成 SKU 目标客群画像。")
        if sku_inputs and not any(item.market_profile for item in sku_inputs):
            warnings.append("M10C 没有读取到 M07 full_observed_window 市场画像，价格带和市场验证降级。")
        if sku_inputs and not any(item.comparable_market_context for item in sku_inputs):
            warnings.append("M10C 没有读取到 M01 周度量价事实，市场验证降级为 M07 累计窗口兼容口径。")
...
class M10CProfileBuilder:
    ...
    def build(
    ...
    ) -> tuple[list[M10CWritePayload], list[M10CWritePayload], list[M10CWritePayload], dict[str, Any]]:
        ...
        for sku_input in sku_inputs:
            sku_scores = [self._score_target_group(sku_input, target_group) for target_group in self.target_groups]
            sku_scores = self._assign_primary_secondary(sku_scores)
            scores.extend(M10CWritePayload(payload) for payload in sku_scores)
            profiles.append(M10CWritePayload(self._profile_payload(sku_input, sku_scores)))
        ...
        coverages = [M10CWritePayload(payload) for payload in self._coverage_payloads(score_payloads)]
        summary = {
            ...
            "taxonomy_codes": [target_group.target_group_code for target_group in self.target_groups],
    ...
    def _score_target_group(self, sku_input: M10CSkuInput, target_group: M10CTargetGroupDefinition) -> dict[str, Any]:
        ...
        claim_match = _claim_match(sku_input.claim_facts, sku_input.claim_profile, target_group)
        param_match = _param_match(sku_input.param_profile, sku_input.market_profile, target_group)
        ...
        payload = {
            "score_id": _score_id(self.project_id, self.batch_id, self.taxonomy.taxonomy_version, sku_input.sku_code, target_group.target_group_code, self.rule_version),
            ...
            "confidence": _confidence(size_price_fit_score, comment_score, task_support_score, claim_score, param_score, sku_input),
            "is_current": True,
        }
    ...
    def _assign_primary_secondary(self, score_payloads: list[dict[str, Any]]) -> list[dict[str, Any]]:
        eligible = [
            ...
            and payload["score_breakdown_json"]["size_price"]["gate_status"] in {"matched", "adjacent"}
        ]
        eligible.sort(
            key=lambda item: (
        ...
        if eligible:
            ...
            eligible[0]["status_reason_cn"] = eligible[0]["status_reason_cn"].replace("可作为候选客群", "作为主目标客群")
            eligible[0]["result_hash"] = _score_result_hash(eligible[0], self.taxonomy.taxonomy_version, self.rule_version)

        secondary = [payload for payload in score_payloads if payload["relation_status"] == REL_SECONDARY]
        ...
        for payload in secondary[3:]:
            ...
            payload["status_reason_cn"] = f"{payload['target_group_name']}证据成立但已超出最多三个次客群限制，降为潜在客群。"
            payload["result_hash"] = _score_result_hash(payload, self.taxonomy.taxonomy_version, self.rule_version)
        return score_payloads

    def _profile_payload(self, sku_input: M10CSkuInput, score_payloads: Sequence[dict[str, Any]]) -> dict[str, Any]:
        ...
        observed = sorted(
            [item for item in score_payloads if item["relation_status"] == REL_COMMENT_OBSERVED],
            key=lambda item: item["target_group_score"],
            reverse=True,
        )
        claimed = sorted(
        ...
        payload = {
            ...
            "price_band_in_size_tier": sku_input.price_band_in_size_tier,
            "price_percentile_in_size_tier": sku_input.price_percentile_in_size_tier,
            "primary_target_group_code": primary["target_group_code"] if primary else None,
            "primary_relation_status": primary["relation_status"] if primary else None,
            "secondary_target_group_codes_json": [item["target_group_code"] for item in secondary],
            "comment_observed_group_codes_json": [item["target_group_code"] for item in observed],
            "brand_claimed_group_codes_json": [item["target_group_code"] for item in claimed],
            "latent_group_codes_json": [item["target_group_code"] for item in latent],
            "unmet_group_need_codes_json": [item["target_group_code"] for item in unmet],
            ...
            "confidence": _avg_decimal([item["confidence"] for item in score_payloads if item["relation_status"] != REL_NOT_SUPPORTED]),
    ...
    def _coverage_payloads(self, score_payloads: Sequence[dict[str, Any]]) -> list[dict[str, Any]]:
        payloads: list[dict[str, Any]] = []
        for target_group in self.target_groups:
            rows = [item for item in score_payloads if item["target_group_code"] == target_group.target_group_code and item["relation_status"] != REL_NOT_SUPPORTED]
            status_counts = Counter(item["relation_status"] for item in rows)
            top_skus = sorted(rows, key=lambda item: item["target_group_score"], reverse=True)[:30]
            payload = {
                "coverage_id": _coverage_id(self.project_id, self.batch_id, self.taxonomy.taxonomy_version, target_group.target_group_code, self.rule_version),
                "project_id": self.project_id,
                "category_code": self.category_code,
                "batch_id": self.batch_id,
                "run_id": self.run_id,
                ...
                "primary_sku_codes_json": [item["sku_code"] for item in rows if item["relation_status"] == REL_PRIMARY],
                "secondary_sku_codes_json": [item["sku_code"] for item in rows if item["relation_status"] == REL_SECONDARY],
                "comment_observed_sku_codes_json": [item["sku_code"] for item in rows if item["relation_status"] == REL_COMMENT_OBSERVED],
                "brand_claimed_sku_codes_json": [item["sku_code"] for item in rows if item["relation_status"] == REL_BRAND_CLAIMED],
                "latent_sku_codes_json": [item["sku_code"] for item in rows if item["relation_status"] == REL_LATENT],
                "unmet_need_sku_codes_json": [item["sku_code"] for item in rows if item["relation_status"] == REL_UNMET],
...
def _filter_target_groups(
    ...
    return tuple(item for item in taxonomy.target_groups if item.target_group_code in code_set)
    ...
    for profile, size_tier in base_inputs:
        ...
        result.append(
            M10CSkuInput(
                ...
                brand_name=(market_profile.brand_name if market_profile else None) or (claim_profile.brand_name if claim_profile else None),
...
def _size_price_gate_status(sku_input: M10CSkuInput, target_group: M10CTargetGroupDefinition) -> str:
    ...
    if size_ok and price_ok:
        return "matched"
    if (size_ok and sku_input.price_band_in_size_tier in target_group.adjacent_price_bands) or (
        price_ok and sku_input.size_tier in target_group.adjacent_size_tiers
    ):
        return "adjacent"
    return "mismatch"
...
def _size_price_fit_score(gate_status: str) -> Decimal:
    return {"matched": Decimal("1.0000"), "adjacent": Decimal("0.5500"), "unknown": Decimal("0.2500")}.get(
        gate_status,
        Decimal("0.0000"),
    )


def _comment_match(comment_facts: Sequence[entities.Core3CommentFactAtom], target_group: M10CTargetGroupDefinition) -> dict[str, Any]:
    wanted = set(target_group.comment_subdimension_codes)
    rows = [
        row
        for row in comment_facts
        if not _is_service_comment(row)
        and (row.subdimension_code in wanted or _keyword_match(row.clean_comment_text, target_group.comment_keywords))
    ]
    service_rows = [row for row in comment_facts if _is_service_comment(row)]
    direct_rows = [row for row in rows if row.dimension_type == "audience_signal" or row.subdimension_code.startswith("audience_")]
    use_case_rows = [row for row in rows if row.dimension_type == "use_case_signal" or row.subdimension_code.startswith("use_")]
    ...
    return {
        ...
        "supported_param_codes": sorted({code for row in rows for code in _list_or_empty(row.supported_param_codes)}),
        "contradicted_param_codes": sorted({code for row in rows for code in _list_or_empty(row.contradicted_param_codes)}),
        "supported_claim_codes": sorted({code for row in rows for code in _list_or_empty(row.supported_claim_codes)}),
        "contradicted_claim_codes": sorted({code for row in rows for code in _list_or_empty(row.contradicted_claim_codes)}),
...
def _is_service_comment(row: entities.Core3CommentFactAtom) -> bool:
    return row.dimension_type == "service_fulfillment_excluded" or row.subdimension_code in SERVICE_SUBDIMENSION_CODES
...
def _claim_match(
    ...
    score = _clamp_decimal(
        (Decimal(len(supported_codes)) + Decimal(max(0, len(claimed_codes - supported_codes))) * Decimal("0.35"))
        / denominator
    )
    return {
        "score": score,
        ...
        "service_excluded_claim_codes": sorted({row.claim_code for row in claim_facts if row.claim_code in wanted and row.service_separate_flag}),
        "evidence_ids": _unique(evidence_id for row in rows for evidence_id in _list_or_empty(row.evidence_ids)),
    }
...
def _brand_trust_boost(comment_facts: Sequence[entities.Core3CommentFactAtom]) -> Decimal:
    rows = [row for row in comment_facts if row.subdimension_code in BRAND_BOOST_CODES and not _is_service_comment(row)]
    if not rows:
...
def _task_support_score(
    *,
    ...
    if comment_direct and comment_score >= Decimal("0.6500") and (claim_score >= Decimal("0.3000") or param_score >= Decimal("0.3000")):
...
def _initial_relation_status(
    *,
    ...
    if negative_count > 0 and negative_count >= max(1, positive_count) and comment_score >= Decimal("0.4500"):
        return REL_UNMET
    if size_price_gate_status == "mismatch" and comment_score < Decimal("0.6500"):
        return REL_NOT_SUPPORTED
    if (
        score >= Decimal("0.5800")
        and (comment_score >= Decimal("0.5500") or task_support_score >= Decimal("0.5500"))
    ...
    if comment_score >= Decimal("0.6500") and (claim_score < Decimal("0.3000") or param_score < Decimal("0.3000")):
        return REL_COMMENT_OBSERVED
    if claim_score >= Decimal("0.5500") and param_score >= Decimal("0.4500") and comment_score < Decimal("0.3500"):
        return REL_BRAND_CLAIMED
    if param_score >= Decimal("0.4500") and size_price_gate_status in {"matched", "adjacent"} and comment_score < Decimal("0.3500"):
        return REL_LATENT
    if score >= Decimal("0.4200") and size_price_gate_status in {"matched", "adjacent", "unknown"}:
        return REL_LATENT
    return REL_NOT_SUPPORTED
...
def _review_reason(
    ...
    if brand_trust_boost > Decimal("0.0000") and not comment_match.get("fact_count") and relation_status == REL_NOT_SUPPORTED:
...
def _comment_summary(score_payloads: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    active = [item for item in score_payloads if item["comment_audience_motivation_score"] > Decimal("0.0000")]
    return {
        "matched_target_group_count": len(active),
        "top_comment_observed": [_compact_score(item) for item in sorted(active, key=lambda item: item["comment_audience_motivation_score"], reverse=True)[:5]],
    }


def _claim_param_summary(score_payloads: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    return {
        "claim_strong_target_group_codes": [item["target_group_code"] for item in score_payloads if item["claim_alignment_score"] >= Decimal("0.5500")],
        "param_strong_target_group_codes": [item["target_group_code"] for item in score_payloads if item["param_capability_score"] >= Decimal("0.5500")],
    }


def _no_primary_reason(score_payloads: Sequence[Mapping[str, Any]], sku_input: M10CSkuInput) -> str:
    if sku_input.size_tier == "unknown" or sku_input.price_band_in_size_tier == "unknown":
        return "缺少 M03B 五档尺寸或 M10C 尺寸内价格带，无法高置信判断主目标客群。"
    active = [item for item in score_payloads if item["relation_status"] != REL_NOT_SUPPORTED]
    if not active:
...
def _score_id(project_id: str, batch_id: str, taxonomy_version: str, sku_code: str, target_group_code: str, rule_version: str) -> str:
    return stable_hash(
        {
            "project_id": project_id,
            "batch_id": batch_id,
...
def _coverage_id(project_id: str, batch_id: str, taxonomy_version: str, target_group_code: str, rule_version: str) -> str:
    return stable_hash(
        {
            "project_id": project_id,
            "batch_id": batch_id,
