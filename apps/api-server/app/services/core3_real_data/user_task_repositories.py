"""M09 user-task repository boundaries."""
...
class M09InputBlockedError(RuntimeError):
    pass


@dataclass(frozen=True)
class M09InputBundle:
...
@dataclass(frozen=True)
class UserTaskRepositoryWriteResult:
    records: tuple[Any, ...]
    created_count: int = 0
    reused_count: int = 0
    updated_count: int = 0
...
class _M09RepositoryMixin(Core3BaseRepository):
        ...
        *,
        ...
        for payload in payloads:
            ...
            if status == "created":
                ...
                reused_count += 1
        return UserTaskRepositoryWriteResult(
    ...
    def _normalize_payload(self, model_cls: Any, payload: Any) -> dict[str, Any]:
        if isinstance(payload, BaseModel):
            ...
            raise TypeError("M09 repository payload must be a mapping or Pydantic model")
...
class M09UserTaskRepository(_M09RepositoryMixin):
    def assert_inputs_ready(self, batch_id: str) -> None:
        required_counts = {
            ...
            "M08 M09 特征视图": self._count_current_rows(
                entities.Core3SkuDownstreamFeatureView,
                batch_id,
                for_module=M08ForModule.M09.value,
            ),
        ...
        if missing:
            raise M09InputBlockedError(f"M09 需要先完成上游产物：{', '.join(missing)}。")

    def list_input_bundles(self, batch_id: str, sku_scope: Sequence[str] = ()) -> list[M09InputBundle]:
        ...
        view_stmt = (
            ...
            .where(entities.Core3SkuDownstreamFeatureView.for_module == M08ForModule.M09.value)
            .order_by(entities.Core3SkuDownstreamFeatureView.sku_code)
        )
        if sku_scope_tuple:
            view_stmt = view_stmt.where(entities.Core3SkuDownstreamFeatureView.sku_code.in_(sku_scope_tuple))
        views_by_sku = {
            view.sku_code: view
            for view in self._paged_scalars(view_stmt, limit=100000, offset=0)
        }

        matrix_stmt = self._current_query(entities.Core3SkuSignalEvidenceMatrix, batch_id).order_by(
        ...
        if sku_scope_tuple:
            matrix_stmt = matrix_stmt.where(entities.Core3SkuSignalEvidenceMatrix.sku_code.in_(sku_scope_tuple))
        matrices_by_sku: dict[str, list[entities.Core3SkuSignalEvidenceMatrix]] = {}
        for matrix in self._paged_scalars(matrix_stmt, limit=100000, offset=0):
            matrices_by_sku.setdefault(matrix.sku_code, []).append(matrix)
    ...
    def save_candidates(self, records: Sequence[Any]) -> UserTaskRepositoryWriteResult:
        return self._save_many(
            entities.Core3SkuTaskCandidate,
            records,
            unique_fields=("batch_id", "sku_code", "task_code", "rule_version", "task_seed_hash"),
        )

    def save_scores(self, records: Sequence[Any]) -> UserTaskRepositoryWriteResult:
        return self._save_many(
            entities.Core3SkuTaskScore,
            records,
            unique_fields=("batch_id", "sku_code", "task_code", "rule_version", "task_seed_hash"),
        )

    def save_breakdowns(self, records: Sequence[Any]) -> UserTaskRepositoryWriteResult:
        return self._save_many(
            entities.Core3SkuTaskEvidenceBreakdown,
            records,
            unique_fields=("batch_id", "sku_code", "task_code", "evidence_domain", "rule_version", "task_seed_hash"),
        )

    def save_review_issues(self, records: Sequence[Any]) -> UserTaskRepositoryWriteResult:
        return self._save_many(
            entities.Core3SkuTaskReviewIssue,
            records,
            unique_fields=("batch_id", "sku_code", "task_code", "issue_type", "input_fingerprint"),
        )

    def list_current_task_scores(
        self,
        batch_id: str,
        *,
        ...
        limit: int = 100,
        offset: int = 0,
    ) -> list[entities.Core3SkuTaskScore]:
        ...
        if sku_code is not None:
            stmt = stmt.where(entities.Core3SkuTaskScore.sku_code == sku_code)
        if task_code is not None:
            stmt = stmt.where(entities.Core3SkuTaskScore.task_code == task_code)
        if relation_level is not None:
            stmt = stmt.where(entities.Core3SkuTaskScore.relation_level == relation_level)
    ...
    def list_current_candidates(
        ...
        *,
        ...
        limit: int = 100,
        offset: int = 0,
    ) -> list[entities.Core3SkuTaskCandidate]:
        stmt = self._current_query(entities.Core3SkuTaskCandidate, batch_id).order_by(
            entities.Core3SkuTaskCandidate.sku_code,
            entities.Core3SkuTaskCandidate.task_code,
        )
        if sku_code is not None:
            stmt = stmt.where(entities.Core3SkuTaskCandidate.sku_code == sku_code)
        if task_code is not None:
            stmt = stmt.where(entities.Core3SkuTaskCandidate.task_code == task_code)
        if candidate_status is not None:
            stmt = stmt.where(entities.Core3SkuTaskCandidate.candidate_status == candidate_status)
        return self._paged_scalars(stmt, limit=limit, offset=offset)
    ...
    def list_current_breakdowns(
        ...
        *,
        ...
        limit: int = 100,
        offset: int = 0,
    ) -> list[entities.Core3SkuTaskEvidenceBreakdown]:
        ...
        if sku_code is not None:
            stmt = stmt.where(entities.Core3SkuTaskEvidenceBreakdown.sku_code == sku_code)
        if task_code is not None:
            stmt = stmt.where(entities.Core3SkuTaskEvidenceBreakdown.task_code == task_code)
        if evidence_domain is not None:
            stmt = stmt.where(entities.Core3SkuTaskEvidenceBreakdown.evidence_domain == evidence_domain)
        return self._paged_scalars(stmt, limit=limit, offset=offset)
    ...
    def list_current_review_issues(
        ...
        *,
        ...
        limit: int = 100,
        offset: int = 0,
    ) -> list[entities.Core3SkuTaskReviewIssue]:
        stmt = self._current_query(entities.Core3SkuTaskReviewIssue, batch_id).order_by(
            entities.Core3SkuTaskReviewIssue.issue_severity.desc(),
        ...
        if sku_code is not None:
            stmt = stmt.where(entities.Core3SkuTaskReviewIssue.sku_code == sku_code)
        if task_code is not None:
            stmt = stmt.where(entities.Core3SkuTaskReviewIssue.task_code == task_code)
        if issue_type is not None:
            stmt = stmt.where(entities.Core3SkuTaskReviewIssue.issue_type == issue_type)
        return self._paged_scalars(stmt, limit=limit, offset=offset)
...
def _stable_identifier_field(model_cls: Any) -> str | None:
    if model_cls is entities.Core3SkuTaskCandidate:
        return "sku_task_candidate_id"
    if model_cls is entities.Core3SkuTaskScore:
        return "sku_task_score_id"
    if model_cls is entities.Core3SkuTaskEvidenceBreakdown:
        return "sku_task_evidence_breakdown_id"
    if model_cls is entities.Core3SkuTaskReviewIssue:
        return "sku_task_review_issue_id"
    return None
