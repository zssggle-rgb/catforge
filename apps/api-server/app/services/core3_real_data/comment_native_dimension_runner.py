"""M08.4 comment-native dimension discovery runner."""
...
from app.services.core3_real_data.comment_native_dimension_schemas import M084ReviewIssueRecord, M084ServiceResult
from app.services.core3_real_data.comment_native_dimension_service import CommentNativeDimensionDiscoveryService
...
class M084CommentNativeDimensionRunner:
    ...
    def run(self, context: Core3RunContext, target: Core3ModuleTarget) -> Core3ModuleRunResultSchema:
        ...
        if not batch_id:
            return _blocked_result(
                ...
                message_cn="M08.4 缺少 M00 batch_id，无法发现评论原生业务维度。",
        ...
        return self.run_batch(
            ...
            rule_version=str(target.metadata.get("rule_version") or CORE3_M08_4_RULE_VERSION),
            seed_version=str(target.metadata.get("seed_version") or CORE3_M08_4_SEED_VERSION),
        )
        ...
        *,
        ...
        try:
            with self.db.begin_nested():
                service_result = CommentNativeDimensionDiscoveryService(
                    CommentNativeDimensionRepository(context)
                ).run_batch(
                    batch_id=batch_id,
                    run_id=run_id,
        ...
        except (M084InputBlockedError, M085DimensionSeedError, ValueError) as exc:
            return _blocked_result(
                project_id=project_id,
                category_code=category_code,
                batch_id=batch_id,
        ...
        summary_json = {
            ...
            "run_id": run_id,
            "batch_id": batch_id,
            "module_version": CORE3_M08_4_MODULE_VERSION,
            "rule_version": rule_version,
            "seed_version": seed_version,
            **service_result.summary,
        }
        output_hash = stable_hash(summary_json, version="m084_comment_native_dimension_summary_v1")
...
def _review_issues(result: M084ServiceResult) -> list[Core3ReviewIssueSchema]:
    return [_review_issue_schema(issue) for issue in result.issues[:100] if issue.severity != "info"]


def _review_issue_schema(issue: M084ReviewIssueRecord) -> Core3ReviewIssueSchema:
    return Core3ReviewIssueSchema(
        issue_code=issue.issue_code,
        issue_type="m084_comment_native_dimension_review",
        severity=_review_severity(issue.severity),
        source_module=Core3ModuleCode.M08_4,
        object_type=issue.object_type,
        object_id=issue.object_code,
        target_sku_code=None,
        evidence_refs=[issue.native_dimension_issue_id],
...
def _downstream_impacts(result: M084ServiceResult) -> list[dict[str, object]]:
    return [
        {
            "module_codes": ["M08.5", "M09", "M10", "M11"],
            "data_domains": [Core3DataDomain.ONTOLOGY.value],
            "reason_cn": "M08.4 评论原生维度发现或预设对齐建议发生变化",
            "native_dimension_count": len(result.candidates),
            "alignment_proposal_count": len(result.alignments),
        }
    ]
