...
from app.services.core3_real_data.candidate_recall_schemas import M12CandidateRecallReviewIssueRecord, M12ServiceResult
from app.services.core3_real_data.candidate_recall_service import CandidateRecallService
...
class CandidateRecallRunner:
    module_code = Core3ModuleCode.M12
    ...
    def run(self, context: Core3RunContext, target: Core3ModuleTarget) -> Core3ModuleRunResultSchema:
        ...
        if not batch_id:
            return _blocked_result(
                ...
                message_cn="M12 缺少 M00 batch_id，无法生成候选池召回结果。",
        ...
        return self.run_batch(
            ...
            rule_version=str(target.metadata.get("rule_version") or CORE3_M12_RULE_VERSION),
        )
        ...
        *,
        ...
        try:
            with self.db.begin_nested():
                service_result = CandidateRecallService(CandidateRecallRepository(context)).run_batch(
                    batch_id=batch_id,
                    run_id=run_id,
        ...
        except (M12InputBlockedError, ValueError) as exc:
            return _blocked_result(
                project_id=project_id,
                category_code=category_code,
                batch_id=batch_id,
        ...
        output_hash = stable_hash(summary_json, version="m12_candidate_recall_summary_v1")
...
def _review_issues(result: M12ServiceResult) -> list[Core3ReviewIssueSchema]:
    return [_review_issue_schema(issue) for issue in result.review_issues[:100]]


def _review_issue_schema(issue: M12CandidateRecallReviewIssueRecord) -> Core3ReviewIssueSchema:
    return Core3ReviewIssueSchema(
        issue_code=issue.issue_type,
        issue_type="m12_candidate_recall_review",
        severity=_review_severity(issue.issue_level),
        source_module=Core3ModuleCode.M12,
        object_type="candidate_pool",
        object_id=issue.candidate_pool_id,
...
def _downstream_impacts(result: M12ServiceResult) -> list[dict[str, object]]:
    return [
        {
            "target_sku_code": pool.target_sku_code,
            "candidate_sku_code": pool.candidate_sku_code,
            "module_codes": ["M13", "M14", "M15"],
            "data_domains": [Core3DataDomain.CANDIDATE.value],
            "reason_cn": "M12 候选池、入池理由或 pair 特征快照发生变化",
        }
...
def _failed_result(
    *,
    ...
    return Core3ModuleRunResultSchema(
        ...
        input_count=0,
        output_count=0,
        warnings=[f"M12 candidate recall failed: {error_message}"],
        summary_json={
            "project_id": project_id,
            "category_code": category_code,
            "batch_id": batch_id,
