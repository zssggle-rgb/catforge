    ...
    return M11CValueBattlefieldTaxonomy(
        ...
        battlefields=(
            _bf(
                ...
                ("use_bedroom", "audience_rental_room", "value_price", "appearance_size_fit"),
                ("tv_claim_value_price", "tv_claim_full_screen_design", "tv_claim_voice_control"),
                ("screen_size_inch", "resolution_class", "smart_tv_flag", "voice_recognition_flag"),
            ),
            _bf(
                ...
                ("audience_senior", "use_bedroom", "use_casting_online", "interaction_voice_casting", "system_smooth_ads"),
                ("tv_claim_voice_control", "tv_claim_casting_connectivity", "tv_claim_ai_large_model"),
                ("voice_recognition_flag", "far_field_voice_flag", "network_tv_flag", "wifi_builtin_flag", "memory_capacity_gb"),
            ...
            _bf(
                ...
                ("use_living_room_cinema", "audience_child_family", "value_price", "picture_clarity_resolution"),
                ("tv_claim_theater_scene", "tv_claim_value_price", "tv_claim_hdr_high_brightness", "tv_claim_full_screen_design"),
                ("screen_size_inch", "resolution_class", "hdr_support_flag", "declared_brightness_nit_or_band"),
            ...
            _bf(
                ...
                ("use_living_room_cinema", "audience_child_family", "picture_clarity_resolution", "picture_eye_care_reflection", "system_smooth_ads", "audio_quality"),
                ("tv_claim_hdr_high_brightness", "tv_claim_wide_color_accuracy", "tv_claim_speaker_sound", "tv_claim_eye_care_display", "tv_claim_voice_control"),
                ("resolution_class", "hdr_support_flag", "declared_refresh_rate_hz", "memory_capacity_gb", "speaker_power_w"),
                adjacent_price_bands=("mid_low", "high"),
            ),
            _bf(
            ...
            _bf(
                ...
                ("use_living_room_cinema", "audio_quality", "picture_clarity_resolution", "picture_brightness_hdr", "appearance_size_fit"),
                ("tv_claim_theater_scene", "tv_claim_hdr_high_brightness", "tv_claim_dolby_audio_video", "tv_claim_speaker_sound"),
                ("screen_size_inch", "hdr_support_flag", "declared_brightness_nit_or_band", "speaker_power_w", "dolby_audio_flag"),
                adjacent_price_bands=("mid_low", "high"),
            ),
            _bf(
                ...
                ("picture_clarity_resolution", "picture_brightness_hdr", "picture_color_accuracy", "picture_local_dimming_black", "use_living_room_cinema"),
                (
            ...
            _bf(
                ...
                ("xlarge_70_85",),
                ("mid", "mid_high"),
                ("TASK_PREMIUM_PICTURE_EXPERIENCE", "TASK_LARGE_SCREEN_UPGRADE", "TASK_VALUE_FOR_MONEY_PURCHASE"),
                ("TASK_CINEMA_IMMERSION",),
                ("TG_VALUE_MAXIMIZER", "TG_PREMIUM_AV_ENTHUSIAST"),
                ("value_price", "picture_clarity_resolution", "picture_brightness_hdr", "competitor_compare"),
                ("tv_claim_value_price", "tv_claim_miniled_display", "tv_claim_hdr_high_brightness", "tv_claim_high_refresh_rate", "tv_claim_speaker_sound"),
                ("display_tech_class", "hdr_support_flag", "declared_brightness_nit_or_band", "declared_refresh_rate_hz", "local_dimming_zone_count"),
            ...
            _bf(
                ...
                ("use_gaming_sports", "gaming_high_refresh_motion", "system_smooth_ads"),
                ("tv_claim_high_refresh_rate", "tv_claim_gaming_low_latency", "tv_claim_hdmi21_connectivity"),
                ("declared_refresh_rate_hz", "hdmi_version_mix", "hdmi_2_1_port_count", "memc_flag", "memory_capacity_gb"),
            ...
            _bf(
                ...
                ("TG_CHILD_FAMILY_LONG_WATCH", "TG_MAINSTREAM_FAMILY_VIEWER", "TG_SENIOR_PARENT_FRIENDLY"),
                ("audience_child_family", "audience_senior", "picture_eye_care_reflection", "use_living_room_cinema"),
                ("tv_claim_eye_care_display",),
                ("hdr_support_flag", "declared_brightness_nit_or_band", "declared_refresh_rate_hz", "eye_care_flag"),
            ...
            _bf(
                ...
                ("use_casting_online", "interaction_voice_casting", "system_smooth_ads", "audience_senior"),
                ("tv_claim_casting_connectivity", "tv_claim_voice_control", "tv_claim_ai_large_model", "tv_claim_smart_home_iot", "tv_claim_camera_interaction"),
                ("wifi_builtin_flag", "network_tv_flag", "smart_tv_flag", "voice_recognition_flag", "far_field_voice_flag", "ai_capability_flag", "ai_model_capability_flag", "smart_home_iot_flag", "camera_flag", "memory_capacity_gb"),
                adjacent_price_bands=("mid_low",),
            ),
            _bf(
            ...
            _bf(
                ...
                ("use_living_room_cinema", "appearance_size_fit", "appearance_slim_wall", "picture_brightness_hdr", "audio_quality"),
                ("tv_claim_theater_scene", "tv_claim_hdr_high_brightness", "tv_claim_local_dimming", "tv_claim_dolby_audio_video", "tv_claim_flush_wall_mount"),
                ("screen_size_inch", "display_tech_class", "hdr_support_flag", "declared_brightness_nit_or_band", "local_dimming_zone_count", "speaker_power_w", "flush_wall_mount_flag"),
                adjacent_size_tiers=("xlarge_70_85",),
                adjacent_price_bands=("mid",),
            ),
        ),
...
class M11CValueBattlefieldRepository(ParamExtractionRepository):
    ...
    def save_scores(self, scores: Sequence[Any], *, replace_on_hash_conflict: bool = False) -> ParamRepositoryWriteResult:
        return self._save_many(
            entities.Core3SkuValueBattlefieldScore,
            scores,
            unique_fields=("batch_id", "taxonomy_version", "sku_code", "battlefield_code", "rule_version", "is_current"),
            hash_field="result_hash",
            replace_existing=replace_on_hash_conflict,
        )

    def save_graph_snapshots(self, snapshots: Sequence[Any], *, replace_on_hash_conflict: bool = False) -> ParamRepositoryWriteResult:
        return self._save_many(
            entities.Core3ValueBattlefieldGraphSnapshot,
            snapshots,
            unique_fields=("batch_id", "taxonomy_version", "rule_version", "is_current"),
...
class M11CInputReader(Core3BaseRepository):
    ...
    def list_market_profiles(self, batch_id: str, sku_codes: Sequence[str]) -> list[entities.Core3SkuMarketProfile]:
        if not sku_codes:
            return []
        stmt = (
    ...
    def list_claim_facts(self, batch_id: str, sku_codes: Sequence[str]) -> list[entities.Core3SkuClaimFact]:
        if not sku_codes:
            return []
        stmt = (
    ...
    def list_comment_profiles(self, batch_id: str, sku_codes: Sequence[str]) -> list[entities.Core3SkuCommentFactProfile]:
        if not sku_codes:
            return []
        stmt = (
    ...
    def list_comment_facts(self, batch_id: str, sku_codes: Sequence[str]) -> list[entities.Core3CommentFactAtom]:
        if not sku_codes:
            return []
        stmt = (
            select(entities.Core3CommentFactAtom)
            .where(entities.Core3CommentFactAtom.project_id == self.project_id)
            .where(entities.Core3CommentFactAtom.category_code == self.category_code.value)
            ...
            .order_by(entities.Core3CommentFactAtom.sku_code, entities.Core3CommentFactAtom.subdimension_code)
        )
        return list(self.db.execute(stmt).scalars())
...
class M11CRunner:
    ...
    def run(self, context: Core3RunContext, target: Core3ModuleTarget) -> Core3ModuleRunResultSchema:
        ...
        return self.run_batch(
            ...
            taxonomy_version=str(target.metadata.get("taxonomy_version") or CORE3_M11C_TV_TAXONOMY_VERSION),
            rule_version=str(target.metadata.get("rule_version") or CORE3_M11C_TV_RULE_VERSION),
    ...
    def run_batch(
        ...
        *,
        ...
        return Core3ModuleRunResultSchema(
            ...
            output_count=service_result.profile_count + service_result.score_count + service_result.graph_snapshot_count,
            output_hash=stable_hash(summary_json, version="m11c_value_battlefield_summary_v1"),
            warnings=service_result.warnings,
            review_issues=[],
            downstream_impacts=[
                {"module_code": "M12", "reason": "价值战场变化会影响竞品库和候选竞品召回。"},
                {"module_code": "M11.5", "reason": "主价值战场会影响卖点溢价/拖后腿分层。"},
            ],
...
class M11CService:
    ...
    def build_batch(
        ...
        *,
        ...
        taxonomy = M11CValueBattlefieldTaxonomyLoader().load(taxonomy_version, product_category=product_category)
        ...
        comment_facts = _group_by_sku(reader.list_comment_facts(batch_id, sku_codes))
        ...
        write_results = {
            "value_battlefield_profiles": repository.save_profiles(profiles, replace_on_hash_conflict=force_rebuild),
            "value_battlefield_scores": repository.save_scores(scores, replace_on_hash_conflict=force_rebuild),
        }
        ...
        return M11CServiceResult(
            ...
            warnings=warnings,
            write_summary={
                key: {"created_count": value.created_count, "reused_count": value.reused_count}
...
class M11CProfileBuilder:
    ...
    def build(
        ...
        *,
        graph_mode: str = "inline",
    ) -> tuple[list[M11CWritePayload], list[M11CWritePayload], M11CWritePayload | None, dict[str, Any]]:
        ...
        for sku_input in sku_inputs:
            sku_scores = [self._score_battlefield(sku_input, battlefield) for battlefield in self.battlefields]
            sku_scores = self._assign_primary_secondary(sku_scores)
            scores.extend(M11CWritePayload(payload) for payload in sku_scores)
            profiles.append(M11CWritePayload(self._profile_payload(sku_input, sku_scores)))
            ...
            size_price_counts[f"{sku_input.size_tier}:{sku_input.price_band_in_size_tier}"] += 1

        graph_snapshot = None
        if graph_mode in {"inline", "rebuild-only"}:
            graph_snapshot = M11CWritePayload(self._graph_payload(sku_inputs, [score.payload for score in scores]))
        summary = {
    ...
    def _score_battlefield(self, sku_input: M11CSkuInput, battlefield: M11CBattlefieldDefinition) -> dict[str, Any]:
        ...
        claim_match = _claim_match(sku_input.claim_facts, sku_input.claim_profile, battlefield)
        param_match = _param_match(sku_input.param_profile, sku_input.market_profile, battlefield)
        user_voice_score = comment_match["score"]
        task_group_fit_score = _task_group_fit_score(comment_match, user_voice_score, market_gate_status)
        claim_alignment_score = claim_match["score"]
        param_capability_score = param_match["score"]
        market_pool_fit_score = {"matched": Decimal("1.0000"), "adjacent": Decimal("0.5500"), "unknown": Decimal("0.2500")}.get(
            market_gate_status,
            Decimal("0.0000"),
        )
        ...
        evidence_ids = _unique(
            [
                ...
                *_list_or_empty(sku_input.market_profile.evidence_ids if sku_input.market_profile else []),
            ]
        )
        ...
        payload = {
            "score_id": _score_id(self.project_id, self.batch_id, self.taxonomy.taxonomy_version, sku_input.sku_code, battlefield.battlefield_code, self.rule_version),
            ...
            "review_required": relation_status in {REL_DRAG, REL_USER_OBSERVED} or market_gate_status == "unknown",
            "review_status": "review_required" if relation_status in {REL_DRAG, REL_USER_OBSERVED} or market_gate_status == "unknown" else "auto_pass",
            "review_reason_json": _json_safe(_review_reason(relation_status, market_gate_status, comment_match, claim_match, param_match)),
            "confidence": _confidence(market_gate_status, user_voice_score, claim_alignment_score, param_capability_score, sku_input),
            "is_current": True,
        }
        payload["result_hash"] = _score_result_hash(payload, self.taxonomy.taxonomy_version, self.rule_version)
        return payload

    def _assign_primary_secondary(self, score_payloads: list[dict[str, Any]]) -> list[dict[str, Any]]:
        eligible = [
            payload
        ...
        eligible.sort(key=lambda item: (item["battlefield_score"], item["user_voice_score"], item["claim_alignment_score"]), reverse=True)
        if eligible:
            ...
            eligible[0]["status_reason_cn"] = eligible[0]["status_reason_cn"].replace("可作为候选战场", "作为主价值战场")
            eligible[0]["result_hash"] = _score_result_hash(eligible[0], self.taxonomy.taxonomy_version, self.rule_version)
        ...
        for payload in secondary[2:]:
            ...
            payload["status_reason_cn"] = f"{payload['battlefield_name']}证据成立但已超出最多两个辅战场限制，降为机会战场。"
            payload["result_hash"] = _score_result_hash(payload, self.taxonomy.taxonomy_version, self.rule_version)
        return score_payloads

    def _profile_payload(self, sku_input: M11CSkuInput, score_payloads: Sequence[dict[str, Any]]) -> dict[str, Any]:
        primary = next((item for item in score_payloads if item["relation_status"] == REL_PRIMARY), None)
        secondary = sorted(
            [item for item in score_payloads if item["relation_status"] == REL_SECONDARY],
            key=lambda item: item["battlefield_score"],
            reverse=True,
        )[:2]
        opportunities = sorted(
            [item for item in score_payloads if item["relation_status"] in {REL_OPPORTUNITY, REL_BRAND_CLAIMED, REL_USER_OBSERVED}],
            key=lambda item: item["battlefield_score"],
            reverse=True,
        )
        drags = sorted(
        ...
        evidence_ids = _unique(evidence_id for item in score_payloads for evidence_id in item["evidence_ids_json"])
        no_primary_reason = None if primary else _no_primary_reason(score_payloads, sku_input)
        battlefield_summary = {
        ...
        payload = {
            "profile_id": _profile_id(self.project_id, self.batch_id, self.taxonomy.taxonomy_version, sku_input.sku_code, self.rule_version),
            "project_id": self.project_id,
            "category_code": self.category_code,
            "batch_id": self.batch_id,
            "run_id": self.run_id,
            ...
            "price_band_in_size_tier": sku_input.price_band_in_size_tier,
            "price_percentile_in_size_tier": sku_input.price_percentile_in_size_tier,
            "primary_battlefield_code": primary["battlefield_code"] if primary else None,
            "primary_relation_status": primary["relation_status"] if primary else None,
            "secondary_battlefield_codes_json": [item["battlefield_code"] for item in secondary],
            "opportunity_battlefield_codes_json": [item["battlefield_code"] for item in opportunities],
            "drag_factor_battlefield_codes_json": [item["battlefield_code"] for item in drags],
            "battlefield_summary_json": battlefield_summary,
            "review_required": bool(no_primary_reason) or any(item["review_required"] for item in score_payloads),
            "review_status": "review_required" if bool(no_primary_reason) or any(item["review_required"] for item in score_payloads) else "auto_pass",
            "review_reason_json": {"no_primary_reason_cn": no_primary_reason} if no_primary_reason else {},
            "confidence": _avg_decimal([item["confidence"] for item in score_payloads if item["relation_status"] != REL_EXCLUDED]),
    ...
    def _graph_payload(self, sku_inputs: Sequence[M11CSkuInput], score_payloads: Sequence[dict[str, Any]]) -> dict[str, Any]:
        active_scores = [item for item in score_payloads if item["relation_status"] != REL_EXCLUDED]
        battlefield_nodes = []
        coverage_summary = {}
        for battlefield in self.battlefields:
            rows = [item for item in active_scores if item["battlefield_code"] == battlefield.battlefield_code]
            status_counts = Counter(item["relation_status"] for item in rows)
            top_skus = sorted(rows, key=lambda item: item["battlefield_score"], reverse=True)[:20]
            ...
            coverage_summary[battlefield.battlefield_code] = {
                ...
                "primary_sku_codes": [item["sku_code"] for item in rows if item["relation_status"] == REL_PRIMARY],
                "secondary_sku_codes": [item["sku_code"] for item in rows if item["relation_status"] == REL_SECONDARY],
                "opportunity_sku_codes": [item["sku_code"] for item in rows if item["relation_status"] in {REL_OPPORTUNITY, REL_BRAND_CLAIMED, REL_USER_OBSERVED}],
                "drag_factor_sku_codes": [item["sku_code"] for item in rows if item["relation_status"] == REL_DRAG],
            }
        sku_nodes = [
            {
        ...
        payload = {
            "graph_snapshot_id": _graph_id(self.project_id, self.batch_id, self.taxonomy.taxonomy_version, self.rule_version),
            "project_id": self.project_id,
            "category_code": self.category_code,
            "batch_id": self.batch_id,
            "run_id": self.run_id,
...
def _filter_battlefields(
    ...
    return tuple(item for item in taxonomy.battlefields if item.battlefield_code in code_set)
    ...
    for profile, size_tier in base_inputs:
        ...
        result.append(
            M11CSkuInput(
                ...
                model_name=profile.model_name or (market_profile.model_name if market_profile else None),
                brand_name=(market_profile.brand_name if market_profile else None) or (claim_profiles.get(profile.sku_code).brand_name if claim_profiles.get(profile.sku_code) else None),
...
def _canonical_size_tier(profile: entities.Core3SkuParamProfile) -> str:
    ...
    tier = ((param_values.get("dimension_tier_profile") or {}).get("size") or "").strip()
    if tier in CANONICAL_SIZE_TIERS:
        return tier
...
def _derive_price_bands(
    ...
    for profile, size_tier in base_inputs:
        ...
        if size_tier in CANONICAL_SIZE_TIERS and price is not None:
...
def _market_gate_status(sku_input: M11CSkuInput, battlefield: M11CBattlefieldDefinition) -> str:
    if sku_input.size_tier == "unknown" or sku_input.price_band_in_size_tier == "unknown":
        return "unknown"
    ...
    if size_ok and price_ok:
        return "matched"
    if (size_ok and sku_input.price_band_in_size_tier in battlefield.adjacent_price_bands) or (
        price_ok and sku_input.size_tier in battlefield.adjacent_size_tiers
    ):
        return "adjacent"
    return "mismatch"


def _comment_match(comment_facts: Sequence[entities.Core3CommentFactAtom], battlefield: M11CBattlefieldDefinition) -> dict[str, Any]:
    ...
    rows = [
        ...
        and (row.subdimension_code in wanted or _comment_text_matches(row.clean_comment_text, battlefield))
    ]
    ...
    return {
        ...
        "sentiment_polarity": _sentiment_polarity(positive_count, negative_count, mixed_count, neutral_count),
        "sample_comments": [row.clean_comment_text for row in rows[:5]],
        "evidence_ids": _unique(evidence_id for row in rows for evidence_id in _list_or_empty(row.evidence_ids)),
    }


def _comment_text_matches(text: str | None, battlefield: M11CBattlefieldDefinition) -> bool:
...
def _task_group_fit_score(comment_match: Mapping[str, Any], user_voice_score: Decimal, market_gate_status: str) -> Decimal:
    matched_subdimensions = set(comment_match.get("matched_subdimension_codes") or [])
    if matched_subdimensions & {"use_living_room_cinema", "use_bedroom", "use_gaming_sports", "use_casting_online", "audience_senior", "audience_child_family", "audience_rental_room"}:
...
def _claim_match(
    ...
    rows = [row for row in claim_facts if row.claim_code in wanted and not row.service_separate_flag]
    supported = [
        row
        for row in rows
        if row.fact_claim_flag and row.param_support_status in {"supported", "partially_supported", "not_param_applicable"}
    ]
    unsupported = [row for row in rows if row.param_support_status == "unsupported_by_param"]
    profile_claim_codes = set(_list_or_empty(claim_profile.fact_claim_codes if claim_profile else [])) & wanted
    ...
    score = _clamp_decimal((Decimal(supported_count) + Decimal(max(0, claimed_count - supported_count)) * Decimal("0.35")) / denominator)
    return {
        "score": score,
        "matched_claim_codes": sorted({row.claim_code for row in rows} | profile_claim_codes),
        "supported_claim_codes": sorted({row.claim_code for row in supported} | profile_claim_codes),
...
def _market_validation_score(
    ...
    if _decimal(market_profile.sales_volume_total) and _decimal(market_profile.sales_volume_total) > Decimal("0"):
...
def _initial_relation_status(
    *,
    ...
    if market_gate_status == "mismatch":
        return REL_EXCLUDED
    if negative_count > 0 and negative_count >= max(1, positive_count) and user_voice_score >= Decimal("0.4000"):
        return REL_DRAG
    if market_gate_status == "matched" and score >= Decimal("0.5800") and user_voice_score >= Decimal("0.3500") and (claim_score >= Decimal("0.3000") or param_score >= Decimal("0.3000")):
        return REL_SECONDARY
    if user_voice_score >= Decimal("0.5500") and (claim_score < Decimal("0.3000") or param_score < Decimal("0.3000")):
        return REL_USER_OBSERVED
    if claim_score >= Decimal("0.5500") and param_score >= Decimal("0.4500") and user_voice_score < Decimal("0.3500"):
        return REL_BRAND_CLAIMED
    if market_gate_status in {"matched", "adjacent", "unknown"} and score >= Decimal("0.4200"):
...
def _value_effect(
    *,
    ...
    if relation_status in {REL_PRIMARY, REL_SECONDARY}:
        if market_gate_status == "matched" and user_voice_score >= Decimal("0.5500") and claim_score >= Decimal("0.5000") and param_score >= Decimal("0.5000"):
    ...
    if relation_status == REL_OPPORTUNITY:
        return VALUE_UNMET if user_voice_score >= Decimal("0.5000") and (claim_score < Decimal("0.3000") or param_score < Decimal("0.3000")) else VALUE_BASIC
    return VALUE_NA
...
def _review_reason(
    ...
    return {
        ...
        "comment_match": {key: value for key, value in comment_match.items() if key != "sample_comments"},
...
def _sentiment_polarity(positive_count: int, negative_count: int, mixed_count: int, neutral_count: int) -> str:
...
def _no_primary_reason(score_payloads: Sequence[Mapping[str, Any]], sku_input: M11CSkuInput) -> str:
    if sku_input.size_tier == "unknown" or sku_input.price_band_in_size_tier == "unknown":
        return "缺少 M03B 五档尺寸或 M11C 尺寸内价格带，无法高置信判断主价值战场。"
    active = [item for item in score_payloads if item["relation_status"] != REL_EXCLUDED]
    if not active:
...
def _user_voice_summary(score_payloads: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    active = [item for item in score_payloads if item["user_voice_score"] > Decimal("0.0000")]
    return {
        "matched_battlefield_count": len(active),
        "top_user_voice": [_compact_score(item) for item in sorted(active, key=lambda item: item["user_voice_score"], reverse=True)[:5]],
    }


def _claim_param_summary(score_payloads: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    return {
        "claim_strong_battlefield_codes": [item["battlefield_code"] for item in score_payloads if item["claim_alignment_score"] >= Decimal("0.5500")],
        "param_strong_battlefield_codes": [item["battlefield_code"] for item in score_payloads if item["param_capability_score"] >= Decimal("0.5500")],
    }
...
def _profile_id(project_id: str, batch_id: str, taxonomy_version: str, sku_code: str, rule_version: str) -> str:
    return stable_hash(
        {
            "project_id": project_id,
            "batch_id": batch_id,
...
def _score_id(project_id: str, batch_id: str, taxonomy_version: str, sku_code: str, battlefield_code: str, rule_version: str) -> str:
    return stable_hash(
        {
            "project_id": project_id,
            "batch_id": batch_id,
...
def _score_result_hash(payload: Mapping[str, Any], taxonomy_version: str, rule_version: str) -> str:
    return stable_hash(
        {
            "sku_code": payload["sku_code"],
...
def _graph_id(project_id: str, batch_id: str, taxonomy_version: str, rule_version: str) -> str:
    return stable_hash(
        {
            "project_id": project_id,
            "batch_id": batch_id,
