...
def _sku_comment_summary(
    ...
    *,
    ...
    candidate_count = _count_sku_rows_where(
        ...
        model.record_status == Core3CleanRecordStatus.ACTIVE.value,
    )
    service_condition = (
        (model.record_status == Core3CleanRecordStatus.SKIPPED.value)
        | model.low_value_reason.contains("服务履约评价")
    )
    ...
    return {
        ...
        "product_comment_candidate_count": candidate_count,
        "service_candidate_count": service_count,
        "service_fulfillment_count": service_count,
        ...
        "service_fulfillment_blocked_count": max(service_count - service_after_low_value_count, 0),
