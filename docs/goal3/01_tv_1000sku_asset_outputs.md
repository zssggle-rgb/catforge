...
Product boundary: CatForge is an internal category asset production line. The outputs below are internal production assets and approved runtime asset packs, not a customer-facing terminal system for client end users.
...
These outputs are internal production deliverables and runtime-pack inputs, not just pipeline logs.
