...
This manual documents the read-only insight interface for parameter profiles, claim fact profiles, market profiles, comment fact profiles, target group profiles, value battlefield profiles, and semantic market graph facts.
...
20. TV semantic market maps and SKU sales allocation.

M12C claim-value quantification is generated by `catforge_pipeline`, but its query surface is `catforge_analyst`, not `catforge_insight`, because it interprets observable price, weekly-sales, and weekly-amount contribution in business terms. For questions such as "哪些卖点是溢价卖点", "某个卖点值多少钱", "某个 SKU 的卖点贡献", "比竞品贵在哪里", or "哪些卖点拖后腿", use `catforge_analyst`.
