...
本文档记录当前整合分支中 M01 的实现。M01 是数据预处理的核心清洗层：它把 M00 登记过的原始行转为可消费的清洗事实，并在这一层拦截空评论、默认评论、低质评论和服务履约类评论。
...
服务履约类评论按“非产品评论”在清洗层拦截，不再新增 `service_fulfillment_comment` 质量类型，也不作为产品质量问题进入后续分析。

M01 对这类评论的处理是：

- `core3_clean_comment` 中保留原始清洗记录。
- `record_status = skipped`，表示不进入后续产品评论分析。
- 不把客服、物流、安装、售后等内容写成产品事实或产品缺陷。
- 进入清洗统计和服务履约拦截统计。
...
```text
客服、物流、安装、售后等服务履约评价已在 M01 识别并拦截，只保留统计，不进入后续产品分析。
```
...
- 服务履约评论当前作为非产品评论在清洗层拦截，不进入后续产品分析。
