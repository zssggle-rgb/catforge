...
本文档记录 `hotfix/data-preprocess-20260618` 分支已经实现的全部开发内容，作为后续对外开放、外部 agent 调用、运维执行和问题排查的参考手册。
...
当前分支：`hotfix/data-preprocess-20260618`
...
- 在 M01 阶段标记为 `low_value_comment`。
- 保留在质量统计中，便于后续评估“有多少评论其实是服务类”。
- 不进入评论句子、M02 产品证据、M05 评论语义分析。
- 不单独新增 `service_fulfillment_comment` 质量类型，避免和服务履约低质口径重复。
...
```text
客服、物流、安装、售后等服务履约评论已在清洗阶段标记为低质评论，
保留在质量统计里，但不会进入后续产品卖点和评论语义分析。
```
...
- 低质评论仍保留在 `core3_clean_comment` 中，用于统计。
...
低质评论不会进入句子切分结果，避免后续 M05/M06 被服务类、空评论和默认好评淹没。
