...
- 当前任务：`M12D-RP-G10` 已完成；任务链无后续 pending 任务
...
8. 用户已明确授权：代码提交、migration、部署、正式全量重跑和 `current/published` 切换均不再单独审批；每次仍必须保留精确提交范围、测试、备份、回滚点和上线验证。
9. 任一品类未通过自身发布门槛时只阻断该品类，不得用另一品类的结果覆盖。
...
8. 提交、部署、正式全量重跑和 `current/published` 切换按任务门槛直接执行，不再单独请求审批。
