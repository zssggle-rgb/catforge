...
### 3.1 M12D 与竞品智能体专项任务链

在原 M00-M16 主链之外，新增一条面向小奥竞品问答质量改造的专项链：

| 专项链 | 调度入口 | 开发任务文档 | 说明 |
| --- | --- | --- | --- |
| M12D SKU成交理由画像 | `M12D_COMPETITOR_goal_dispatch.md` | `M12D_development_tasks.md` | 独立完成 SKU 级成交理由画像的分析、实现、小批量验证、全量生成和版本发布。 |
| 竞品智能体消费 M12D | `M12D_COMPETITOR_goal_dispatch.md` | `CATFORGE_ANALYST_competitor_answer_development_tasks.md` | 在 M12D 已发布基础上，改造关键价值锚点可替代性、替代压力、报告结构、看板和小奥 Skill。 |

调度原则：

- M12D 生产链路先于竞品智能体正式消费链路。
- 竞品智能体不得在自身流程里生成 M12D。
- 定时器每次只执行一个任务编号，任务完成后更新专项调度入口和对应任务文档。
