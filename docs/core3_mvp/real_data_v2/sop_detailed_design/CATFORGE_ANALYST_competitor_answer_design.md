...
角色槽位只用于确定“哪三款进入 Top 3”，不得直接作为最终名次。集合确定后必须无条件按报告展示的整数综合分（六个计分维度整数分之和）降序重新排名，同分时依次比较直接竞争关系、市场验证等级、购买池得分和价格接近度。关键价值锚点和替代压力门槛只能限制竞品角色及强替代话术，不能改变名次。因此报告中的“综合分”和“排名”必须保持一致，不能出现低分候选排在高分候选之前。
