...
1. 服务履约类直接排除。
2. `source_type=competitor_gap` 进入竞品拦截/机会缺口，不进入本品正向价值榜。
3. 弱相关卖点退出该战场正向量化。
4. 门槛校验先于高溢价判断。
5. 支撑参数竞争力校验先于高溢价、份额转化和客户获得价值判断。
6. 可比池按 L0-L3 放宽；放宽后仍不足才降级为样本不足。
7. 再根据市场位置、卖点价值分和证据链分类。

分类规则：

| 分类 | 必要条件 |
| --- | --- |
| 稳定高溢价卖点 | `source_type=target_fact_claim`；市场位置为 `premium_accepted`；卖点价值分 >= 75；不是门槛；参数竞争力为 `leading` 或 `strong`；评论不弱；样本不是 `sparse_unknown` |
| 高溢价候选/需复核 | 参数竞争力强，但评论、样本、市场验证或文本型号等级表存在不足 |
| 份额转化卖点 | `source_type=target_fact_claim`；市场位置为 `share_conversion`；卖点价值分 >= 70；销量或销额强；参数竞争力不弱 |
| 客户获得价值卖点 | `source_type=target_fact_claim`；市场位置为 `customer_value_gain`；卖点价值分 >= 70；价格低但销量强；参数竞争力不弱 |
| 人无我有型支付价值卖点 | `source_type=target_fact_claim`；本品有事实卖点或关键参数领先；同战场可比池缺少有效对照、目标 SKU 是有卖点组孤例，或对照组不足；战场相关且评论/参数至少一项强 |
...
| MT9655/画质芯片/处理器性能 | 人无我有型支付价值卖点 | 与画质处理、控光、高刷和智能体验相关，参数和评论有支撑；但同战场同价位缺少足够对照，不能量化金额 |
...
| 护眼显示、杜比/影音认证、影院/观影场景 | 待激活/舒适体验 | 可作为体验解释，但需要护眼、影音任务和评论验证，不能只靠认证文本 |
...
| MT9655/画质芯片/处理器性能 | 人无我有型支付价值卖点 | 暂不量化金额 | 支撑画质处理和高阶体验，但同战场对照样本不足；输出潜力等级、参数证据和验证条件 |
...
1. 重构 M12C 结果模型，增加 `source_type`、单战场结果、参数竞争力快照和 SKU 汇总结果。
2. 实现本品事实卖点、参数能力、竞品机会缺口的拆分，保证正向榜只来自本品事实卖点和本品参数能力。
3. 实现卖点-支撑参数映射，生成 `ClaimParameterEvidence`。
4. 实现可比市场池和样本等级，保留 L0-L3 放宽轨迹。
5. 实现参数竞争力快照，覆盖数值型、档位型、布尔型、类目型、文本型号型参数。
6. 实现门槛卖点先行校验，尤其是 MiniLED、基础 HDR、基础高刷、基础 HDMI2.1。
7. 实现卖点价值分和分类规则，确保高溢价必须经过参数竞争力门槛。
8. 实现单战场金额/销量空间和 SKU 层战场权重汇总。
9. 实现 CLI 原子能力。
10. 实现 Skill 路由和短答模板。
11. 实现 Markdown/飞书报告模板，分开展示本品卖点价值与竞品机会缺口。
12. 用 65E7Q 完成回归验收，重点验证 5200nits、1920 分区进入稳定高溢价，MiniLED/HDMI2.1 不进入高溢价 Top。
