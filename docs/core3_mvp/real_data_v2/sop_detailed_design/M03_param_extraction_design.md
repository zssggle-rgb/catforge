...
> 2026-06-20 设计更新：M03 已拆分为 M03A 和 M03B 两个生命周期不同的能力。M03A 负责从某个品类的原始字段生成可复核、可发布的参数语义 taxonomy；M03B 负责在新数据批次上消费已发布 taxonomy，生成 SKU 参数事实画像。M03A 详细设计见 `M03A_param_taxonomy_semantic_asset_design.md`。本文保留为现有 M03/M03B 参数抽取与 SKU 参数画像设计基础，后续实现不应继续把静态彩电 seed 当作所有品类的默认参数体系。
