...
购买阻力不是竞品识别评分维度，不进入 100 分制，也不在“分析过程”目录中设置独立章节。只有双方存在同一已成立理由、且两侧阻力均不是 `unassessed` 时，才在关键价值锚点或产品画像下以“补充说明（不计分）”横向展示；无共同成立理由、任一侧未评估或仅有占位结论时整段省略。`proposition_only/rejected/weak_expression` 不进入同理由压力比较。
