...
- M12C 行存在但金额不可量化时，仍可作为对应 AC 购买理由的定性价值证据；只读取当前 AC 锚点引用的 claim，未引用卖点和 TV claim 不得影响该锚点。
...
## 11. AC ContextBuilder 输入质量要求

- AC 必须使用与 TV 相同的 typed 质量契约，但 availability/usability 判断、问题到锚点的映射必须来自 AC taxonomy，不能复用 TV claim、参数或购买理由 code。
- 新上市、观察期不足、零销量和线上渠道范围是 AC 市场画像事实，不是量价数据不完整；只有当前 AC serving scope 内市场画像行真实不存在才为 missing。
- 匹数、形态、能效、舒适风、新风、静音和维护相关问题只影响引用该证据的 AC 锚点；局部 warning 不得将全部 AC 购买理由统一降级。
- AC M12C 只读取 `m12c_ac_claim_value_quantification_v0.2`，并按当前 AC 购买理由引用的 claim code 聚合。金额不可量化只限制金额/WTP 表达，不删除定性价值证据。

## 12. AC 锚点评分要求

- AC 的 `ac_claim_*` 必须先按 AC taxonomy 映射到当前购买理由，再读取该 claim 的 M12C role；其他 AC claim 的 `drag_factor` 或价格拦截角色不得扩散。
- 冷暖效果解释价格、长期省电抵价、同价位获得感和低价核心体验属于 AC 价格理由，需当前锚点 M12C，或评论感知与匹数/形态价格池市场承接同时成立。
- 匹数空间匹配、大空间冷暖、睡眠静音、柔风、新风、除湿、清洁、安装和智能控制属于功能/风险降低理由，可由 AC 参数、评论、场景和市场强证据独立成立，不把 M12C 作为全局硬门槛。
- 零销量和弱市场承接可以作为当前价格理由的业务风险，但不能解释为数据缺失，也不能连坐不依赖市场承接的功能理由。
- 同一市场证据域的 `market_pool_*`、`price_band_sample_*`、`size_pool_*` 只扣一次；info 永不扣分，blocking 命中时不得成为 core。
