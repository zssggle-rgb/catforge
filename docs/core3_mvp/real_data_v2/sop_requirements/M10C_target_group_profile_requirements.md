...
量价事实按完整观测口径消费：某周没有销量记录表示该周销量为 0，新上市 SKU 观测周数较短是生命周期事实，不是数据缺失。M10C 不得另设“至少 8 周”等固定在售周门槛来降低目标客群画像质量。
...
SKU 目标客群画像是否可用只由主客群成立质量决定，不得把任一非主客群关系的提醒扩散到整张画像。

自动通过必须同时满足：

1. 存在且仅存在一个 `primary_target_group`。
2. 主客群关系状态为 `primary_target_group`。
3. 主客群关系置信度 `>= 0.8333`，即 6 个证据域中至少 5 个成立。
4. 主客群与尺寸价格适配不冲突。
5. 不存在综合分、评论动机分、任务支撑分、卖点对齐分四项排序键完全相同的另一主客群候选。

需要整画像复核的情形仅包括：无主客群、主关系状态不一致、主客群置信度低于 `0.8333`、主客群尺寸价格冲突或完整排序键并列。只比较综合分相近不构成歧义；排序键可以客观区分时应选出主客群。

以下问题只保留在对应 SKU x 客群关系行，不降低整张画像：次要/观察/厂家主打/潜在/未满足客群需复核、服务信号被正确排除、非主客群存在未知参数或卖点代码、非主客群支撑不足。

TV 与 AC 共用上述质量传播原则，但继续使用各自独立 taxonomy、客群 code、尺寸价格口径和规则版本；不得复用另一品类的客群定义。
...
| `review_reason_json` | 仅记录画像作用域的主客群问题；关系级问题留在 SKU x 客群分数行 |
...
11. TV/AC 中主客群置信度不低于 `0.8333`、尺寸价格不冲突且无完整排序键并列的画像自动通过。
12. 无主客群、主关系异常、主客群低置信、尺寸价格冲突或完整排序键并列的画像仍需复核。
13. 切换质量聚合规则前后，客群 taxonomy、主次客群 code、关系状态、关系分数和证据保持不变。
