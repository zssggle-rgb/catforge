...
本文件已按“单模块逐一强化”要求完成第一轮强化。下一步应处理 M08 SKU 综合信号画像。

## 1. 模块目标

M07 把清洗后的周量价数据转换成 SKU 市场画像、价格/销量/销额分位、市场信号和可比池基线，为后续任务、客群、价值战场、战场内卖点价值分层、候选召回和竞品评分提供市场证据。

M07 要解决五个问题：

1. 把 `26W01` 到 `26W23` 这类周销售事实，沉淀成 SKU 级市场画像。
2. 在当前只有线上渠道、2 个平台、35 个型号的样例数据下，建立可解释的可比池。
3. 输出价格带、尺寸段、平台重合、销量/销额分位、趋势和样本充分性。
