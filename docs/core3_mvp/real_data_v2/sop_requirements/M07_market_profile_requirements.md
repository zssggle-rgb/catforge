...
以下规则自 `m07_market_profile_v2` 起生效，并覆盖本文后续仍可能出现的旧版“有效周门槛”表述：

1. 周量价数据按完整市场日历解释；SKU 在某周没有记录，表示该周销量和销额为 0，不表示数据缺失。
2. 新上市 SKU 历史不足 8 周、SKU 距最近成交周较远，都是生命周期或市场表现事实，不得作为数据质量问题，也不得触发画像降级。
3. `full_observed_window`、`latest_week`、`recent_4w/8w/12w` 均以品类数据集的全局日历边界计算，不按 SKU 的有成交周压缩窗口。
4. 基期销量为 0 或某窗口无成交时，销量增长率或成交价变化率可以为不可计算；这是指标适用性，不是数据缺失。
5. 少于 52 周和仅线上渠道以结构化范围 `info` 表达，不进入 `quality_flags`、不触发 `review_required`、不扣市场置信度。
6. 品类集合必须同时满足 `category_code` 和稳定 SKU 前缀校验；TV 池不得消费 `AC...`，AC 池不得消费 `TV...`。
7. 分块运行只限制目标 SKU，候选池始终使用该批次、该品类的完整候选集合，分块大小不得改变分位和池成员。
...
| 窗口 | 说明 |
| --- | --- |
| `full_observed_window` | 品类数据集最小周至全局最新周，SKU 无记录周补 0 |
| `latest_week` | 品类数据集全局最新周；SKU 当周无成交则销量/销额为 0、成交价不可计算 |
| `recent_4w` | 以全局最新周回溯 4 个日历周 |
| `recent_8w` | 以全局最新周回溯 8 个日历周 |
| `recent_12w` | 以全局最新周回溯 12 个日历周 |
...
| `price_latest` | 全局最新周的成交加权均价；当周零销量时不可计算，不回填历史成交价 |
| `price_median` | 周均价中位数 |
| `price_min` | 观察期最低有效均价 |
| `price_max` | 观察期最高有效均价 |
| `active_week_count` | 有成交记录的市场活跃周数，仅描述活跃度，不表示数据覆盖度 |
| `latest_week` | 最近成交周，作为市场活跃事实 |
...
趋势按完整日历周补 0。只有整个数据集的日历范围不足以覆盖比较窗口时，才将趋势标记为不适用；新品自身成交周少不构成样本不足。
...
空调品类不使用屏幕尺寸，必须使用 M03B 的安装形态、匹数和制冷量生成 AC 市场分层：

| AC 分层 | 规则 |
| --- | --- |
| 挂机 | `wall_hp_1_or_below`、`wall_hp_1_5`、`wall_hp_2`、`wall_hp_3` |
| 柜机 | `floor_hp_2`、`floor_hp_3` |
| 3匹及以上柜机 | 3匹柜机和 3匹以上/大制冷量柜机统一归入 `floor_hp_3`，业务展示为“3匹及以上柜机” |
| 价格带 | 在同一匹数段内按加权均价分位生成 `low/mid_low/mid/mid_high/high` |
| 同池统计 | AC 的 `market_pool_key`、`same_pool_sku_count` 和同池价格/销量/销额分位必须按“匹数段 × 价格带 × 渠道 × 窗口”计算，不得只按匹数段计算 |

该规则用于解决两个样本问题：3匹以上柜机不能被拆成单 SKU 池；1.5匹挂机样本过大时，必须通过匹数段内价格带继续拆分，避免同池统计失去区分度。
...
- SKU 入选市场批次后，无周记录按销量/销额 0 处理；零销量窗口没有成交价是正常市场事实，必须与显式空值、非法数值和尺寸缺失区分。
...
- 品类市场时间轴整体不可用。
- 价格、销量、销额无法数值化或明显异常。
- 全局最新周零销量导致当前成交价不可计算时只展示事实，不进入复核。
- 尺寸缺失，无法进入尺寸可比池。
- 可比池样本数过少。
- 业务价格区间或业务尺寸区间内 SKU 数过少，区间内位置只能低置信展示。
- 整个数据集日历范围不足以覆盖趋势比较窗口。
- 平台占比异常集中或平台字段缺失。
- 某 SKU 市场画像较上一批波动异常。
