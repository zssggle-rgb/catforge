...
用户任务不是参数、卖点、评论主题或价格带的直接标签。评论代表用户真实目的和体验，是最高优先级；卖点代表厂家想打什么；参数代表产品能力能否支撑；尺寸价格代表该任务是否处在合理竞争池中；销量和销额只能做验证。
...
| `market_validation_score` | 销量、销额、价格/英寸和价格分位是否验证该任务 |
