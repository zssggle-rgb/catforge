...
量价事实按完整观测口径消费：某周没有销量记录表示该周销量为 0，新上市 SKU 观测周数较短是生命周期事实，不是数据缺失。M09C 不得另设“至少 8 周”等固定在售周门槛来降低用户任务画像质量。
...
SKU 画像是否可用只由主任务成立质量决定，不得把任一任务关系的提醒直接扩散到整张画像。

自动通过必须同时满足：

1. 存在且仅存在一个 `primary_user_task`。
2. 主任务关系状态为 `primary_user_task`。
3. 主任务关系置信度 `>= 0.80`。
4. 主任务没有达到阻断阈值的尺寸价格冲突或用户/能力反证。

需要整画像复核的情形仅包括：无主任务、主关系状态不一致、主任务置信度低于 `0.80`、主任务存在阻断级证据冲突。以下情况只保留在对应 SKU x 任务关系行，不降低整张画像：

- 次要任务、评论观察任务、厂家主打任务、潜在能力任务或拖后腿任务需要复核。
- 服务语句被正确排除。
- 某个非主任务引用了 taxonomy 中当前没有映射到 SKU 的参数或卖点代码。
- 非主任务存在评论、卖点或参数支撑不足。

TV 与 AC 共用上述质量传播原则，但继续使用各自独立 taxonomy、任务 code、尺寸价格口径和规则版本；不得复用另一品类的任务定义。
...
| `review_reason_json` | 仅记录画像作用域的主任务问题；关系级问题留在 SKU x 任务分数行 |
