...
8. 评论匹配必须体现强度差异，不得只要达到强支持就把不同战场都记为满分。评论句数、正负向、子维度覆盖、是否表达购买/使用理由都要进入评分。
9. 产品体验类评论必须能反推用户任务和目标客群。例如“高清、画质清晰、色彩真实、亮度高、控光好”在高价/中大尺寸池中应支持“高端画质体验”和“高端影音体验用户”，不能只作为产品体验证据。
10. 当多个战场分数接近时，必须按业务主支付理由做主战场兜底判定，避免微小公式差异把辅能力误判为主竞争理由。
...
量价事实按完整观测口径消费：某周没有销量记录表示该周销量为 0，新上市 SKU 观测周数较短是生命周期事实，不是数据缺失。M11C 不得另设“至少 8 周”等固定在售周门槛来降低价值战场画像质量。
...
`user_voice_score` 不能使用简单的“有/无评论”或“达到阈值即满分”逻辑。每个 SKU × 战场的评论分必须至少考虑：

| 因素 | 业务含义 |
| --- | --- |
| 匹配评论数量 | 用户在该战场下真实讨论的集中度 |
| SKU 内相对强度 | 同一 SKU 的多个战场中，哪个战场获得更多用户声音 |
| 子维度覆盖 | 是否覆盖清晰度、色彩、亮度、流畅、音效、尺寸、投屏等多个体验点 |
| 正负向结构 | 正向集中、负向集中、正负混合的价值含义不同 |
| 购买/使用理由直达度 | 评论是否直接表达“为什么选它”“用来做什么”“是否值” |

同一 SKU 中，32 条高端画质相关评论和 8 条游戏/流畅相关评论不得得到完全相同的用户声音分。评论多且集中在付费理由上的战场，应在主战场判定中获得更高权重。

### 4.2 体验评论到任务客群的映射要求

评论不只包含显式用途词。M11C 必须把品类核心体验评论映射到对应任务和客群：

| 评论事实 | 可支撑用户任务 | 可支撑目标客群 |
| --- | --- | --- |
| 画质清晰、高清、色彩真实、亮度高、控光好、黑位好 | 高端画质体验、影院沉浸观影 | 高端影音体验用户、主流家庭观影用户 |
| 画面流畅、不卡、刷新率、游戏性能、看球、运动画面 | 主机游戏娱乐、体育赛事观看 | 游戏体育娱乐用户 |
| 音效好、电影感、沉浸、大片、家庭影院 | 影院沉浸观影 | 高端影音体验用户、巨幕家庭影院用户 |
| 系统流畅、投屏方便、语音、联网稳定、智能功能实用 | 投屏互联与智能控制、长辈易用操作 | 智能互联用户、长辈友好用户 |
| 不刺眼、不反光、看着舒服、孩子/老人长时间看 | 长时间护眼观看 | 儿童家庭长看用户、长辈友好用户 |

这类映射必须结合尺寸价格门槛、参数能力和卖点支撑使用。不能因为一句“画质好”就让低价小屏进入高端画质战场；但在中大尺寸高价池且高端画质参数/卖点成立时，画质体验评论必须能增强高端画质任务和高端影音客群。

### 4.3 主战场近分兜底要求

当两个或多个候选战场满足主/辅条件且综合分差小于 `0.03` 时，M11C 不得只按小数点排序确定主战场，必须执行业务兜底：

1. 优先选择评论证据更集中、且评论更接近购买/支付理由的战场。
2. 若评论强度接近，优先选择参数和卖点能完整解释该 SKU 高价/高销量位置的战场。
3. 若仍接近，优先选择与 SKU 主尺寸价格池更一致的战场。
4. 辅助能力型战场不得压过更能解释 SKU 支付理由的主价值战场。

示例：海信 65E7Q 在 65 寸高价池中，高端画质评论、参数和卖点更集中，游戏体育流畅能力也成立。若两者综合分接近，应判定 `BF_PREMIUM_PICTURE_UPGRADE` 为主战场，`BF_GAMING_SPORTS_FLUENCY` 为强辅战场。

### 4.4 画像可用性与复核作用域

SKU 价值战场画像是否可用只由主战场成立质量决定，不得把任一非主战场关系的提醒扩散到整张画像。

自动通过必须同时满足：

1. 存在且仅存在一个 `primary_battlefield`。
2. 主战场关系状态为 `primary_battlefield`。
3. 主战场关系置信度 `>= 0.80`，即 5 个证据域中至少 4 个成立。
4. 主战场满足本品类市场门槛：TV 必须 `matched`；AC 可以是 `matched`，或满足 AC 强证据规则的 `adjacent`。
5. 不存在购买理由排序键和原始战场排序键全部相同的另一主候选。

主战场近分不等于歧义。先执行既有购买理由兜底，再使用原始战场排序；只有两阶段完整排序键都不能区分候选时，才标记 `primary_battlefield_ambiguous`。

以下问题只保留在对应 SKU x 战场关系或后续锚点，不降低整张画像：辅战场、机会战场、厂家主打战场、用户观察战场需复核；拖后腿战场及其负向评论；非主战场参数/卖点不足。拖后腿战场必须继续进入 `drag_factor_battlefield_codes` 和后续相关锚点风险，不能因画像自动通过而删除。

TV 与 AC 共用上述质量传播原则，但继续使用各自独立 taxonomy、市场门槛、战场 code 和规则版本。
...
| `review_reason_json` | 仅记录画像作用域的主战场问题；relation/anchor 问题留在对应战场分数和后续锚点 |
...
`score_breakdown_json` 必须写入评论证据拆分，至少包含 `comment_fact_count`、`positive_comment_count`、`negative_comment_count`、`relative_comment_intensity_score`、`matched_subdimension_codes`、`direct_value_reason_count`、`task_group_inference_reason`。这样业务复核时能看出为什么某战场是主战场，为什么另一个只是辅战场。
...
11. TV/AC 中主战场置信度不低于 `0.80`、市场门槛合法且两阶段排序无并列的画像自动通过。
12. 无主战场、主关系异常、主战场低置信、市场门槛冲突或两阶段完整排序并列的画像仍需复核。
13. 辅助、机会、用户观察和拖后腿战场不降低整画像，但 relation 与 anchor 风险完整保留。
14. 切换质量聚合规则前后，taxonomy、主辅战场 code、关系状态、关系分数、销量分配和证据保持不变。
