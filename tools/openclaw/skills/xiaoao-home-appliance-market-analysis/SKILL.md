...
4. Product profiles: for each of the four SKUs, write `市场画像`,
   `价值战场画像`, `用户任务画像`, `目标客群画像`, `卖点画像`, and `参数画像`.
   Claim profiles must distinguish premium claims, basic support claims,
   sales-support claims, basic threshold claims, opportunity gaps,
   drag-factor claims, review-needed claims, and quantified claim-value fields
   when available. Quantified fields include `卖点溢价指数`, `可观测价格支撑`,
   `可观测周均销量支撑`, and confidence.
