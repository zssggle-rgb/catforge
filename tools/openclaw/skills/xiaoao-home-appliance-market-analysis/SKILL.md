...
| "某 SKU 的用户卖点价值是什么", "卖点支付价值", "卖点价值有哪些" | `sku-claim-value` | `sku_code` or `query` | Broad SKU-level claim-value question. In Feishu card entry, call the stable XiaoAo card command with `--with-report feishu-doc --feishu-card-only`, so the visible answer is the card delivery status and the user sees the claim-value dashboard card. Do not request JSON, do not run jq/grep/Python post-processing, and do not make multiple exploratory tool calls. |
